...
<div class=" container mx-auto px-4 sm:px-12">
    ...
    <section id="crearSubproceso" class="mb-4 mt-8">
        <strong>
            Seguro cambiaste de parecer y tienes algunos cambios.
        </strong>
        <p>Para empezar necesitamos actailizar los datos del puesto de trabajo <strong>{{$subprocesse->name}}</strong> .
        </p>

        <form        
            method="POST"
            action="{{route('subprocesse.update', $subprocesse)}}"
        >

            <x-inputinfo value="{{old('name', $subprocesse->name)}}">
